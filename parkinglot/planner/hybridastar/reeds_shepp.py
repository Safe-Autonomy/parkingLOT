...
based on: https://github.com/AtsushiSakai/PythonRobotics/blob/master/PathPlanning/ReedsSheppPath/reeds_shepp_path_planning.py
"""
import math
...
def straight_left_straight(x, y, phi):
    phi = mod2pi(phi)
    comp_thresh = 0.01
    # only take phi in (comp_thresh*math.pi, (1-compu_thresh)*math.pi) for the sake of speed.
    ...
    if math.pi * comp_thresh < phi < math.pi * (1-comp_thresh) and y != 0:
        xd = - y / math.tan(phi) + x
